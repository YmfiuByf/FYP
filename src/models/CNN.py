...
class my_newCNN(nn.Module):
    def __init__(self, n_mfcc=15, kernel_length=[], hidden_layer=[], Cin=3, Cout=256, k=[],output_size = 6):
        super().__init__()
...
def train_CNN(test_mode=False,n_mfcc=70,method='inception',
              ...
              lr=1e-4,weight_decay=0.1,dropout =0.7,batch_size=16):
    ...
    if not test_mode:
        if method =='CNNv1':
            folder = rf"E:\FYP excel\{dataset}_CNN{str}_kernel={kernel_length}_linear={hidden_layer}_k={k}_seed{seed}_mfcc{n_mfcc}_{pooling}pooling"
    ...
    if CNN is None:
        if method == 'CNNv1':
            CNN = my_newCNN(n_mfcc=n_mfcc,kernel_length=kernel_length, hidden_layer=hidden_layer, Cin=3, Cout=256, k=k).to(device)
