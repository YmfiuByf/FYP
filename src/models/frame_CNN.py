...
class SER_Inception(nn.Module):
    ...
    def _forward(self, x: Tensor,f0:Tensor) -> Tensor:
        # N x 3 x 299 x 299
    ...
    def forward(self, x: Tensor, f0:Tensor) -> inception.InceptionOutputs:
        x = self._transform_input(x)
        x = self._forward(x,f0)
        return x
...
# pooling = 'max'
lr,weight_decay,dropout = 1e-3, 0.1, 0.7
...
l = length
def extract_shuffle(df,df0,output_size,num,keep_length=20):
    X_ =None
    def unison_shuffled_copies(a, b):
        assert len(a) == len(b)
        p = np.random.permutation(len(a))
        return a[p], b[p]
    for i in trange(1,output_size+1):
        arr=np.array(df[df['label']==i])
        keep = len(arr) // keep_length * keep_length
        X = arr[:keep,1:-1]
        arr_f0 = np.array(df0[df0['label']==i].iloc[:keep,1])
        arr_f0[np.where(np.isnan(arr_f0))]=0

        if attention:
            X = X.reshape((keep//keep_length,-1))
            arr_f0=arr_f0.reshape((keep//keep_length,-1))
            X,arr_f0 = unison_shuffled_copies(X,arr_f0)
            X = X.reshape((keep,-1))
            arr_f0 = arr_f0.reshape((keep,-1)).squeeze()


        # starts = np.random.randint(len(X)-length,size=num//output_size)
        starts = np.array( random.sample(range(len(X)-length),num//output_size) )
        # starts = np.array(random.sample(range(len(X)//length-1),num//output_size))*length + np.random.randint(-2,2,size=num//output_size)
        Xs,f0s = [],[]
        for start in tqdm(starts):
            Xs.append(X[start:start+l,:].reshape((l, n_mfcc, 3), order='F'))
            f0s.append(arr_f0[start:start+l])
        X = np.array(Xs)
        f0 = np.array(f0s)
        y = np.ones(len(f0))*i
        if i==1:
            ...
            f0_ =np.concatenate([f0_,f0],axis=0)
    X_ = np.moveaxis(X_,-1,1).astype(np.float32)
    f0_ = f0_.astype(np.float32)[:,None,:]
    y_ = y_-1
    return train_test_split(X_,y_,f0_,test_size=0.2)

df = pd.read_csv(csv_path1)
df0 = pd.read_csv(csv_f01)
X_train,X_test,y_train,y_test,f0_train,f0_test = extract_shuffle(df,df0,output_size,num=num)
...
#     return X, y, starts,np.array(df_f0.iloc[:,1],dtype=np.float32)
#
#
# X_train,y_train,starts_train,f0_train = load_dataset(csv_path1,csv_f01)
# X_test,y_test,starts_test,f0_test = load_dataset(csv_path2,csv_f02)
#
# # Fixed Length Feature
# def get_feature(X,y,starts,f0,l=length):
#     cnn_features = []
#     labels = []
#     f0s = []
#     i,j = 0,l
...
#     f0s[np.where(np.isnan(f0s))]=0.
#     return X, y, f0s[:,None,:]
#
# def get_feature_train_val(X,y,starts,f0,l=length,val_size=0.1):
#     global seed
...
#     y_val = y_val - 1
#     f0s_val = np.array(f0s_val, dtype=np.float32)
#     f0s[np.where(np.isnan(f0s))] = 0.
#     f0s_val[np.where(np.isnan(f0s_val))] = 0.
#     print(np.unique(y_val, return_counts=True))
#     return X, y, f0s[:,None,:], X_val,y_val,f0s_val[:,None,:]
# if test_mode:
#     X_test, y_test,f0_test = get_feature(X_test, y_test, starts_test,f0_test)
#     X_train,y_train,f0_train = get_feature(X_train,y_train,starts_train,f0_train)
#     X_train = X_train[:160]
#     y_train = y_train[:160]
#
# else:
#     X_train, y_train,f0_train,X_test,y_test,f0_test = get_feature_train_val(X_train, y_train, starts_train,f0_train)
#     while (len(np.unique(y_test)) < output_size):
#         seed += 1
#         X_train, y_train, starts_train,f0 = load_dataset(csv_path1, csv_f01)
#         X_train, y_train,f0_train,X_test,y_test,f0_test = get_feature_train_val(X_train, y_train, starts_train,f0_train)
#         print(f'seed={seed}')
...
# del X,y
dl_train = DataLoader(TensorDataset(X_train,y_train,f0_train), batch_size=batch_size, shuffle=True)
dl_test = DataLoader(TensorDataset(X_test,y_test,f0_test), batch_size=batch_size, shuffle=True)
print('loader built')
...
def Train(model, dl_train,dl_test,loss_function, metric_function, optimizer, epochs=150, batch_size=32,model_name='CNN_dd',df_path=folder+f'\\{dataset}_{method}_{attention}_MFCC{n_mfcc}.csv'):
    ...
    for epoch in range(len(dfhistory)+1,epochs+1):
        ...
        CM_val = torch.zeros((output_size, output_size)).int().to(device)
        for step, (features,y_true,f0) in enumerate(dl_train,1):
        ...
        for val_step, (features,labels,f0) in enumerate(dl_test,1):
            with torch.no_grad():
                # if len(features) > 1:
                #     y_pred = torch.tensor([]).to(device)
                #     for i in range(len(features)):
